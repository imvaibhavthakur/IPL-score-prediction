...
st.markdown("<h1 style='text-align: center; color: white;'> IPL ScoreWizard 2024 </h1>", unsafe_allow_html=True)
...
def validate_over(overs):
    if 0 <= overs - math.floor(overs) <= 0.6:
        return True
    else:
        return False

with col1:
    overs = st.number_input('Enter the Current Over', min_value=5.0, max_value=19.6, value=5.0, step=0.1, format="%.1f", key="overs", help="Valid range: 5.0 to 19.5")
    if not validate_over(overs):
        st.error('Please enter a valid over input (0 to 6) as one over only contains 6 balls')
with col2:
#Enter Current Run
...
if st.button('Predict Score'):
    ...
    st.markdown("<div style='background-color: rgba(255, 251, 204, 0.5); padding: 10px; border-radius: 5px;'><strong><span style='color:#8a6d3b'>!</span> Factors such as batting team's performance, pitch conditions, weather, and more significantly influence the predicted cricket score.</strong></div>", unsafe_allow_html=True)
